...
export default function ChatbotWidget() {
  ...
  async function send() {
    ...
    setLoading(true);
    try {
      const data = await api<{ answer: string; needs_followup?: boolean }>(
        "/message",
        { method: "POST", body: JSON.stringify({ message: userMsg.content }) },
      );
      ...
      setMessages((m) => [
        ...
        {
          role: "assistant",
          content: "Sorry, I ran into a problem. Please try again later.",
          time: Date.now(),
        },
      ]);
    } finally {
        ...
        <div className="fixed bottom-24 right-6 z-50 w-[min(92vw,380px)] rounded-xl border bg-white shadow-xl flex flex-col overflow-hidden">
          <div className="px-3 py-2" style={{ background: BLUE }}>
            ...
            <div className="text-white/70 text-xs">
              Ask about login, profile, events, donations, mentorship
            </div>
          </div>
