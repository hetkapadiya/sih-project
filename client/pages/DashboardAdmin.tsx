...
export default function DashboardAdmin() {
  ...
  const toggleAdmin = (id: string) => {
    const raw = localStorage.getItem("alumnihub_users") || "[]";
    const list = JSON.parse(raw) as any[];
    const updated = list.map((u) => (u.id === id ? { ...u, role: u.role === "admin" ? "alumni" : "admin" } : u));
          ...
          {users.map((u: any) => (
            <div key={u.id} className="flex items-center justify-between p-3 bg-white rounded-md border">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => toggleAdmin(u.id)} className="px-3 py-1 rounded-md bg-primary text-primary-foreground text-sm">{u.role === "admin" ? "Revoke Admin" : "Make Admin"}</button>
                <button onClick={() => deleteUser(u.id)} className="px-3 py-1 rounded-md bg-destructive text-destructive-foreground text-sm">Delete</button>
              </div>
            </div>
          ))}
        </div>
