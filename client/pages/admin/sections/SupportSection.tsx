...
export default function SupportSection() {
  ...
  React.useEffect(() => {
    (async () => {
      try { const f = await api<{ items: { q: string; a: string }[] }>("/faqs"); setFaqs(f.items); } catch {}
      try { const l = await api<{ logs: any[] }>("/logs"); setLogs(l.logs); } catch {}
    })();
  }, []);
  ...
  async function addFaq() {
    if (!faqQ || !faqA) return;
    await api("/faqs", { method: "POST", body: JSON.stringify({ q: faqQ, a: faqA }) });
  ...
  return (
    <div className="grid gap-4">
      ...
      <Card>
        <CardHeader><CardTitle>Chatbot FAQs</CardTitle></CardHeader>
        <CardContent className="grid gap-2">
          <div className="grid md:grid-cols-2 gap-2">
            <Input placeholder="Question (keywords)" value={faqQ} onChange={(e) => setFaqQ(e.target.value)} />
            <Input placeholder="Answer" value={faqA} onChange={(e) => setFaqA(e.target.value)} />
          </div>
          ...
          <Table className="mt-2">
            ...
            <TableBody>
              {faqs.map((f, i) => (
                <TableRow key={i}>
                  <TableCell className="max-w-[260px] truncate">{f.q}</TableCell>
                  <TableCell className="max-w-[360px] truncate">{f.a}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      <Card>
        <CardHeader><CardTitle>Chat Logs</CardTitle></CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {logs.map((l, i) => (
                <TableRow key={i}>
                  <TableCell>{new Date(l.at || Date.now()).toLocaleString()}</TableCell>
                  <TableCell className="max-w-[260px] truncate">{l.message}</TableCell>
                  <TableCell className="max-w-[360px] truncate">{l.answer}</TableCell>
                </TableRow>
              ))}
              {logs.length === 0 && (
                <TableRow><TableCell colSpan={3} className="text-sm text-muted-foreground">No logs yet.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
